function check_equilibration(folder::String, nat_msa::Array{Int8,2}, step_msa::Array{Array{Int8,2},1}, steps::Array{Int,1})
    ...
    close("all")
    plt.plot(steps, (err_dist .^2)./L, label = "Simulated"); plt.plot(steps, [err_dist_nat^2 for _ in 1:N_msa] ./L, label = "Natural"); plt.legend(); plt.xlabel("MCMC steps"); plt.ylabel("Variance hamming from wt (%)");plt.xscale("log"); savefig(joinpath(folder,"var_hamming.png"))
